...
$stmt = $conn->prepare("SELECT s.*, b.salon FROM services s JOIN business_users b ON s.business_id = b.id WHERE s.id = ?");
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['confirm_booking'])) {
    ...
    if ($booking_date && $booking_time) {
        $insert = $conn->prepare("INSERT INTO bookings (service_id, salon_id, customer_id, customer_name, booking_date, booking_time, created_at)
                                  VALUES (?, ?, ?, ?, ?, ?, NOW())");
        $insert->execute([
            $service_id,
            $salon_id,
...
<div class="max-w-xl mx-auto bg-white p-8 rounded shadow">
   <h2 class="text-xl font-bold mb-4">Book <?= htmlspecialchars($service['title']) ?></h2>
